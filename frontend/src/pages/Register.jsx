...
export default function Register() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', confirm: '', role: 'citizen', inviteCode: '' })
  const [error,   setError]   = useState('')
  const [loading, setLoading] = useState(false)
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true)
    try {
      const user = await register(form.name, form.email, form.password, form.role, form.inviteCode)
      navigate(user.role === 'officer' ? '/municipal' : '/dashboard', { replace: true })
    } catch (err) {
      setError(err.response?.data?.error || 'Registration failed. Please try again.')
    } finally {
      ...
      <div style={{ width: '100%', maxWidth: '440px' }}>
        ...
        <div className="card" style={{ padding: '32px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                {[
                  ...
                  <button
                    key={r.value}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, role: r.value, inviteCode: '' }))}
                    style={{
                      padding: '12px',
                      borderRadius: 'var(--radius-md)',
                      border: `1px solid ${form.role === r.value ? 'rgba(251,191,36,0.4)' : 'var(--border-default)'}`,
            ...
            {/* Officer invite code — only shown when officer is selected */}
            {form.role === 'officer' && (
              <div className="form-group" style={{ animation: 'fadeInUp 0.2s var(--ease-out)' }}>
                <label className="form-label">Officer Invite Code *</label>
                <input
                  className="form-input"
                  type="password"
                  name="inviteCode"
                  placeholder="Enter your municipal invite code"
                ...
                <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                  This code is provided by your municipal authority.
                </span>
              </div>
            )}
